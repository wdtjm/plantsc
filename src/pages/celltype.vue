...
export default {
  ...
  mounted() {
        ...
        this.$request.post('/api/getMarker.php', {
          ...
          species: this.species
            ...
            .then((response) => {
              ...
              this.$eventBus.$emit('celltype-marker-table', response['data'])
    ...
    eventBus.$on('search-celltype', (selects) => {
      console.log('search-selects', selects);
      this.species = selects['species']
      this.getTreeData(selects['species']);
      eventBus.$emit('marker', selects['celltype']);
      /* eventBus.$emit('intro',node); */
      eventBus.$emit('treeClick', selects['tissue']);
    })
  },
  beforeDestroy() {
    eventBus.$off('marker');
    eventBus.$off('search-celltype');
  }
}
</script>
