...
$treeData = file_get_contents('../../data/plants.json');
// 解码JSON数据
...
function calculateNum($species,$celltype,$conn,$tissue){
    try {
        ...
        $sql = "SELECT COUNT(DISTINCT `Marker genes`) AS count FROM Sheet1 WHERE Species = '$species' AND Celltypes LIKE '$celltype_all' AND Tissues = '$tissue'";
...
/* for($j = 0; $j < count($treeArray); $j++){
    $name=$treeArray[$j]['name'];
    for ($i = 0; $i < count($phpArray); $i++) {

    tree_travel($phpArray[$i],$name,$pdo,$phpArray[$i]['lable']);
    tree_travel_reverse($phpArray[$i]);
    $phpArray[$i]['nodeNum']=tree_travel_calculate_not_null_node($phpArray[$i]);
    }

    // 打印解码后的数据

    file_put_contents('../../data/tree/' . $name . '.json', json_encode($phpArray));
} */
